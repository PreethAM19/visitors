             ...
             <form method="post"  action= "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" > <!--<?php // echo htmlspecialchars($_SERVER["PHP_SELF"]);?>-->
               ...
               <?php  if($category==$category_constants["cab"]){?>
                 ...
                 <div class="mb-3 input-box" id="nameContainer">
                     ...
                     <label for="name"><?php if($category==$category_constants["visitor"]||$category==$category_constants["delivery"]){echo 'Visitor Name';}else{echo 'Service Name';};?></label>
                ...
                <!--<?php if(($category==$category_constants["visitor"])||($category==$category_constants["delivery"])||($category==$category_constants["cab"])){?>
				 ...
				 <!-- Added by Preetham -1/14/23 -->
				  <div class="form-group">
					...
					<div class="form-group">
                        <label for="SFMobile">Host (Faculty/Student) Mobile Number</label>
                        <input type="text" id='SFMobile'name="sf_phone_number" class="form-control" placeholder="Enter Student/Faculty Mobile Number." required value='<?php echo $sf_phone_number ?>'/>
						<p id="sfPhoneNumberErrMsg" name="sf_phone_number_err_msg" class="error-message"><?php echo $sf_phone_number_err_msg; ?></p>
                        <!--div id="whomToMeetError" class="error-message"></div-->
                    </div>
               ...
               <!--  <div id="otherAddressContainer" style="display: flex; flex-wrap: wrap; justify-content:between;">
				...
				<!--	<select id="otherAddress" class="select-field mb-3" name="other_address" ></select>
                 ...
                 <div class="mb-3 input-box">
                     ...
                     <label for="purpose">Purpose/Comments</label>
				 ...
				 <!--added for attachments option -->
								
				<div class="mb-3 input-box">
					
					
						 <label for="camera">Capture Photo (Optional)</label> <br>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/selectize.js/0.14.0/js/standalone/selectize.min.js"></script>
             ...
             <script type="text/javascript">
				 ...
				 let sfPhoneNumber = document.getElementById("SFMobile");

                 let phoneNumberErrMsg = document.getElementById("phoneNumberErrMsg");
				 
				 let sfPhoneNumberErrMsg = document.getElementById("sfPhoneNumberErrMsg");
				...
				let delServContainer = document.getElementById("delServContainer");
				 ...
				 let wtm = document.getElementById("whomToMeet");
            ...
            //    }
             ...
             //*************************************************************************************************************** */
                ...
                //  })
				 ...
				 sfPhoneNumber.addEventListener("keypress",function(event){

                     sfPhoneNumberErrMsg.textContent = "";
                      ...
                      if(event.keyCode>47 && event.keyCode<58){
                         ...
                         sfPhoneNumber.value = event.target.value

                     }else{  

                         event.preventDefault()                                            

                         sfPhoneNumberErrMsg.textContent = "Enter a valid Phone Number"}

                     

                     if(sfPhoneNumber.value.length>=10){

                         event.preventDefault()

                         sfPhoneNumberErrMsg.textContent = "Can Enter Only 10 digits"
					 }						 
...
////var $delServiceSelectize = $('#delService').selectize({
  ...
  //  }
			  ...
			  var whomToMeetOptions = <?php echo json_encode($wtmoptions); ?>;
				
				var $wtmSelectize = $('#whomToMeet').selectize({
					create: true,
					openOnFocus: true,
					sortField: 'text',
			...
			var wtmSelectize = $wtmSelectize[0].selectize;
				for (var i = 0; i < whomToMeetOptions.length; i++) {
					wtmSelectize.addOption({value: whomToMeetOptions[i], text: whomToMeetOptions[i]});
				}
            ...
            $.post('send_otp.php', {action: 'generate_otp', mobile: phoneNumber}, function(response) {
        ...
        function verifyOTP() {
            let otp = $('#otp').val();
            $.post('send_otp.php', {action: 'verify_otp', otp: otp}, function(response) {
                let result = JSON.parse(response);
                alert(result.message);
                if (result.status == 'success') {
                    $('#otpModal').hide();
		...
		document.getElementById('file').addEventListener('change', function() {
			...
			if (file && !allowedTypes.includes(file.type)) {
				alert('Invalid file type. Please upload PDF, PNG, or JPG/JPEG.');
