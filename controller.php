...
$sql="select entityname, userid  from pw_entity where entitytype='EMPLOYEE' and status='Active' and hremptype<>'' order by hremptype,entityname"; 
  
//Current active students list:

$sql1= "select entityname, userid  from pw_entity where entitytype in ('Academics Students','Temporary Students') and entitycode not like '110%' and currentstatus='Active' order by entityname asc";
...
array_push($wtmoptions,$row['entityname']);
//echo "<br/>";
...
$name = $phone_number  = $sf_phone_number = $point_of_contact =  $vehicle_status =  $vehicle_type = $vehicle_number =$image_path_to_save = $cab_service_name= $other_service_name=$other_name=$other_address=$otherAddressSelectize=$del_service=$other_room=$delServiceSelectize=$image_file_name="";

$name_err_msg = $last_name_err_msg=$phone_number_err_msg= $sf_phone_number_err_msg= $point_of_contact_err_msg=$department_err_msg=$vehicle_type_err_msg=$vehicle_number_err_msg=$visitor_image_err_msg=$other_service_name_err_msg=$other_address_err_msg=$other_name_err_msg=$wtm="";

$image_uploaded_status = false;

$to_store_data_in_db = ["name"=>"","phone_number"=>"","sf_phone_number"=>"","whomToMeet"=>"","vehicle_type"=>"","vehicle_number"=>"","purpose"=>""]; //"department_to_approach"=>"",
...
unset($_SESSION['back_trigger_webcam_page']);
   ...
   $name=$_SESSION[$data_in_ls_with_key]["name"];
    ...
    if(($category==$category_constants["visitor"])||($category==$category_constants["delivery"])||($category==$category_constants["other_services"])){
	 ...
	 $sf_phone_number=($_SESSION[$data_in_ls_with_key]["sf_phone_number"]);
...
if(isset($_POST["next_on_page_one"])){
 ...
 //var_dump($_POST);
    ...
    if(($category==$category_constants["visitor"])){
		...
		$sf_phone_number = ($_POST["sf_phone_number"]);
...
//
 ...
 if(empty($sf_phone_number)){

    $to_store_data_in_db["sf_phone_number"]="Null";  

 }else{

  $sf_phone_number = test_input($sf_phone_number);

    $to_store_data_in_db["sf_phone_number"]=$sf_phone_number;

    $sf_phone_number_err_msg="";
...
//   }
  ...
  $to_store_data_in_db["sf_phone_number"]!="" && 
